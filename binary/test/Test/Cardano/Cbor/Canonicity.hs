...
traverseCanonicalBits
  ...
  -- ^ negative int
  -> m Word8
  -- ^ big integer (add leading zeroes)
  -> (UInt -> m UInt)
  -- ^ length of a string/collection
  -> (UInt -> m UInt)
  -- ^ tag
...
perturbCanonicity term = sized $ \sz -> do
  n        <- choose (1, sz)
  toChange <-
    shuffle . take (countCanonicalBits term) $ replicate n True ++ repeat False
  evalStateT
    (traverseCanonicalBits tuint tnint tzeroes tlength ttag tmapset tnan16 term)
    toChange
 where
  ...
  tzeroes :: StateT [Bool] Gen Word8
  tzeroes = shouldBeChanged >>= \case
    False -> pure 0
    True  -> lift . sized $ \sz -> fromIntegral <$> choose (1, sz)
  ...
  tmapset :: [t] -> StateT [Bool] Gen [t]
  tmapset terms = shouldBeChanged >>= \case
    False -> pure terms
    True  -> lift $ oneof [shuffle terms, addDuplicate terms]
   where
  ...
  widenUInt = \case
    ...
    UInt16 w -> elements [UInt32 $ fromIntegral w, UInt64 $ fromIntegral w]
