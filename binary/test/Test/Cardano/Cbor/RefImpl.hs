...
instance Functor Decoder where
  fmap f (Decoder d) = Decoder $ fmap (first f) . d
...
getByte = Decoder $ \case
  w : ws' -> Just (w, ws')
  _       -> Nothing

getBytes :: Integral n => n -> Decoder [Word8]
getBytes n = Decoder $ \ws -> case genericSplitAt n ws of
  (ws', [])
    | genericLength ws' == n -> Just (ws', [])
    | otherwise              -> Nothing
  (ws', ws'') -> Just (ws', ws'')
...
decodeInitialByte ib =
  (toEnum $ fromIntegral $ ib `shiftR` 5, fromIntegral $ ib .&. 0x1f)

prop_InitialByte :: Property
prop_InitialByte = conjoin
  [ (uncurry encodeInitialByte . decodeInitialByte) w8 === w8
...
encodeAdditionalInfo = enc
 where
  ...
  enc (AiValue (UInt64 w)) = (27, [w7, w6, w5, w4, w3, w2, w1, w0])
    where (w7, w6, w5, w4, w3, w2, w1, w0) = word64ToNet w
  enc AiIndefLen = (31, [])
  enc (AiReserved n)
    | n >= 28 && n < 31 = (n, [])
...
prop_AdditionalInfo ai =
  let
    (w, ws)       = encodeAdditionalInfo ai
    Just (ai', _) = runDecoder (decodeAdditionalInfo w) ws
  in ai === ai'


data TokenHeader =
  TokenHeader MajorType
...
encodeTokenHeader (TokenHeader mt ai) =
  let (w, ws) = encodeAdditionalInfo ai in encodeInitialByte mt w : ws

prop_TokenHeader :: TokenHeader -> Property
prop_TokenHeader header =
  let
    ws                = encodeTokenHeader header
    Just (header', _) = runDecoder decodeTokenHeader ws
  in header === header'

prop_TokenHeader2 :: Property
prop_TokenHeader2 = conjoin
  [ w8 : extraused === encoded
  | w8 <- [minBound .. maxBound]
  , let
    extra :: [Word8]
    extra                 = [1 .. 8]
...
instance Arbitrary Token where
  arbitrary = oneof
   ...
   where
    arbitraryLengthUInt :: [Word8] -> Gen UInt
    arbitraryLengthUInt xs =
      ...
      in
        ...
        ++ [UInt32 (fromIntegral n), UInt64 (fromIntegral n)]
...
packToken (TokenHeader mt ai) extra = case (mt, ai) of
  ...
  (MajorType7, AiValue (UInt8 w)) -> return (MT7_Simple w)
...
encodeToken tok =
  let (header, extra) = unpackToken tok in encodeTokenHeader header ++ extra
...
prop_Token token =
  let
    ws                = encodeToken token
    Just (token', []) = runDecoder decodeToken ws
  in token `eqToken` token'
 where
    -- NaNs are so annoying...
  eqToken :: Token -> Token -> Property
  eqToken (MT7_Float16 f) (MT7_Float16 f') | isNaN f && isNaN f' =
    property True
  eqToken (MT7_Float32 f) (MT7_Float32 f') | isNaN f && isNaN f' =
    property True
  eqToken (MT7_Float64 f) (MT7_Float64 f') | isNaN f && isNaN f' =
    property True
  eqToken a b = a === b
...
integerToBinaryRep zs n =
  replicate (fromIntegral zs) 0
    ++ (integerToBytes $ if n >= 0 then n else -1 - n)
...
instance Arbitrary Term where
  arbitrary = frequency
    ...
    , (1, TNInt <$> arbitrary)
    , ( 1
      , arbitrary >>= \(LargeInteger n) -> do
        bs <- sized $ \sz ->
          integerToBinaryRep <$> (fromIntegral <$> choose (0, sz)) <*> pure n
    ...
    , ( 1
      , arbitrary
        >>= \bs -> TString <$> arbitraryLengthRep (encodeUTF8 bs) <*> pure bs
      )
    , (1, TStrings <$> arbitrary)
    , ( 2
      , listOfSmaller arbitrary
        >>= \arr -> TArray <$> arbitraryLengthRep arr <*> pure arr
      )
    , (2, TArrayI <$> listOfSmaller arbitrary)
    , ( 2
      , listOfSmaller ((,) <$> arbitrary <*> arbitrary)
        >>= \arr -> TMap <$> arbitraryLengthRep arr <*> pure arr
      )
    , (2, TMapI <$> listOfSmaller ((,) <$> arbitrary <*> arbitrary))
    , ( 1
      , TTagged <$> arbitraryTag <*> sized
        (\sz -> resize (max 0 (sz - 1)) arbitrary)
      )
    , (1, pure TFalse)
    , (1, pure TTrue)
    , (1, pure TNull)
   ...
   where
    ...
    arbitraryLengthRep xs
      ...
      | len <= 0xffffffff = elements
        [UInt32 $ fromIntegral len, UInt64 $ fromIntegral len]
  ...
  shrink (TBigInt bs len n) =
    [ let bs' = integerToBinaryRep zs n' in TBigInt bs' (replaceLen bs' len) n'
  ...
  shrink (TString n ws) =
    [ TString (replaceLen (encodeUTF8 ws') n) ws' | ws' <- shrink ws ]
  shrink (TStrings wss) = [ TStrings wss' | wss' <- shrink wss ]

  shrink (TArray n xs@[x]) =
    x : [ TArray (replaceLen xs' n) xs' | xs' <- shrink xs ]
  ...
  shrink (TMap n xys@[(x, y)]) =
    x : y : [ TMap (replaceLen xys' n) xys' | xys' <- shrink xys ]
  shrink (TMap n xys) = [ TMap (replaceLen xys' n) xys' | xys' <- shrink xys ]
  shrink (TMapI xys@[(x, y)]) = x : y : [ TMapI xys' | xys' <- shrink xys ]
  shrink (TMapI xys) = [ TMapI xys' | xys' <- shrink xys ]

  shrink (TTagged w t) =
    [ TTagged w' t' | (w', t') <- shrink (w, t), not (reservedTag (fromUInt w')) ]
...
decodeTermFrom = \case
  ...
  MT3_String n ws ->
    either (const decodeErr) (return . TString n) $ decodeUTF8 ws
...
decodeBytess acc = decodeToken >>= \case
...
decodeStrings acc = decodeToken >>= \case
  ...
  MT3_String _ ws -> do
    cs <- either (const decodeErr) return (decodeUTF8 ws)
    decodeStrings (cs : acc)
  _ -> decodeErr

decodeArrayN :: UInt -> Decoder Term
decodeArrayN n =
  TArray n <$!> replicateM (fromIntegral $ fromUInt n) decodeTerm
...
decodeMapN n = TMap n <$!> replicateM
  (fromIntegral $ fromUInt n)
...
encodeTerm (TBytess wss) =
  encodeToken MT2_ByteStringIndef
    <> mconcat
        [ encodeToken (MT2_ByteString len ws)
        | ws <- wss
...
encodeTerm (TString len cs) =
  let ws = encodeUTF8 cs in encodeToken (MT3_String len ws)
encodeTerm (TStrings css) =
  encodeToken MT3_StringIndef
    <> mconcat
    ...
    <> encodeToken MT7_Break
encodeTerm (TArray len ts) =
  encodeToken (MT4_ArrayLen len) <> mconcat (map encodeTerm ts)
encodeTerm (TArrayI ts) =
  encodeToken MT4_ArrayLenIndef
    <> mconcat (map encodeTerm ts)
    <> encodeToken MT7_Break
encodeTerm (TMap len kvs) = encodeToken (MT5_MapLen len)
  <> mconcat [ encodeTerm k <> encodeTerm v | (k, v) <- kvs ]
encodeTerm (TMapI kvs) =
  encodeToken MT5_MapLenIndef
    <> mconcat [ encodeTerm k <> encodeTerm v | (k, v) <- kvs ]
    <> encodeToken MT7_Break
encodeTerm (TTagged tag t) = encodeToken (MT6_Tag tag) <> encodeTerm t
...
prop_Term term =
 ...
 where
  ...
  eqTerm (TArray n ts) (TArray n' ts') =
    n == n' .&&. conjoin (zipWith eqTerm ts ts')
  eqTerm (TArrayI ts) (TArrayI ts') = conjoin (zipWith eqTerm ts ts')
  eqTerm (TMap n ts) (TMap n' ts') =
    n == n' .&&. conjoin (zipWith eqTermPair ts ts')
...
word16ToNet w =
  ( fromIntegral ((w `shiftR` 8 {- 8*1 -}
                               ) .&. 0xff)
  , fromIntegral ((w `shiftR` 0 {- 8*0 -}
                               ) .&. 0xff)
  )
...
word64ToNet
  :: Word64 -> (Word8, Word8, Word8, Word8, Word8, Word8, Word8, Word8)
word64ToNet w =
...
prop_word16ToFromNet w1 w0 = word16ToNet (word16FromNet w1 w0) === (w1, w0)
...
fromFloat float = unsafeDupablePerformIO $ alloca $ \buf -> do
...
newtype FloatSpecials n = FloatSpecials
  { getFloatSpecials :: n
  } deriving (Show, Eq)
...
instance (Arbitrary n, RealFloat n) => Arbitrary (FloatSpecials n) where
  arbitrary = frequency
    ...
    , ( 1
      , pure (FloatSpecials (1 / 0))
      )  -- +Infinity
    , ( 1
      , pure (FloatSpecials (0 / 0))
      )  --  NaN
...
instance Arbitrary LargeInteger where
  arbitrary = sized $ \n -> oneof $ take
    ...
    , LargeInteger . bigger . fromIntegral <$> choose
