...
import           Cardano.Chain.Update (ApplicationName (..), BlockVersion (..),
                     ...
                     UpId, UpdateData (..), Vote (..), VoteId, mkProof,
                     mkVoteSafe, signProposal)
...
exampleBlockVersionData = BlockVersionData
  ...
  where
    ...
    c2'   = Coeff (MkFixed 77)
    sfrule =
        SoftforkRule (CoinPortion 99) (CoinPortion 99) (CoinPortion 99)
...
exampleProposalBody = ProposalBody bv bvm sv hm ua
  where
    ...
    hm =
        Map.fromList $ zip (exampleSystemTags 10 5) (exampleUpdateDatas 10 5)
