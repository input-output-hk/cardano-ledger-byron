...
elaborate config (_, _, pps) ast st ab
  ...
  where
    ...
    prevHash
      = fromMaybe (Genesis.getGenesisHash $ Genesis.configGenesisHash config)
...
annotateBlock block =
  let res =
        case Binary.decodeFullDecoder "Block" Concrete.decodeABlockOrBoundary bytes of
          Left err ->
            panic $ "This function should be able to decode the block it encoded"
                  <> ". Instead I got: " <> show err
          Right abobb ->
            map (LBS.toStrict . Binary.slice bytes) abobb
  in
  case res of
    Concrete.ABOBBlock bk   ->
      bk
