...
-- (Cardano.Chain.Txp.Core).

module Test.Cardano.Chain.Txp.CoreSpec
       ( spec
       ) where
...
import           Cardano.Chain.Txp (Tx (..), TxIn (..), TxOut (..), checkTx,
                     topsortTxs)
import           Cardano.Core (mkCoin)
import           Cardano.Core.Attributes (mkAttributes)
import           Cardano.Crypto (hash, whData, withHash)
import           Cardano.Util (_neHead)

import           Test.Cardano.Chain.Txp.Arbitrary ()
import           Test.Cardano.Util.QuickCheck.Arbitrary (sublistN)
  ...
  where
    ...
    continueGraph vertices unusedUtxo reachable k
      ...
      | otherwise =  do
        ...
        (Cardano.tive outputsN) <- resize some' arbitrary
        -- gen some outputs
...
txGen :: Int -> Gen Tx
txGen size = do
    (Cardano.tive inputsN) <- resize size arbitrary
    (Cardano.tive outputsN) <- resize size arbitrary
