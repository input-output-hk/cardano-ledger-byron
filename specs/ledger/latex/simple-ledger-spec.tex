...
The following section shows how the ledger rules are used to build the
blockchain ones.
...
\begin{figure}
  ...
  \begin{equation}
    ...
    \inference[Chain-ext]
    {\var{dmap}~\var{vk_g} = vk & \bwit{b} = (\var{vk}, \sigma) & \var{vk_g} \in \Gkeys\\
      \var{cslot} < \bslot{b} & \size{[k \mid k \in signers,~ k = \var{vk_g}]} \leq K * t &
      \verify{vk}{\serialised{\bbody{b}}}{\sigma} \\
      {\begin{array}{l}
        \var{cslot}\\
        d
      \end{array}}
      ...
      \trans{bdeleg}{b}
      {
        \left(
          \begin{array}{l}
            ...
            \var{utxo}\\
            \var{dmap'}\\
            \var{signers} ~\mathbf{;_K}~ \var{vk_g}\\
            \bslot{b}\\
            \var{pdlgs'}\\
            \var{ekeys'}            
          \end{array}
  ...
  \begin{equation*}
    ...
    \left(
      \begin{array}{r@{~\in~}lr}
        ...
        \var{cslot} & \SlotId & \text{current slot}\\
...
\begin{figure}
  \centering
  \begin{equation}
    \label{eq:rule:sequence-delegation}
  \end{equation}
