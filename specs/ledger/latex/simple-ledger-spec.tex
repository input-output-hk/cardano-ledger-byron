...
\item[Update validation] voting mechanism which captures the identification of
  the voters, and the participants that can post update proposals.\todo{Do we model
  this and is this the right place for it?}
\end{description}
...
\item[Delegation] delegation certificates are posted to the blockchain, and
  hence they are not part of the ledger.
\end{description}
...
we leave the definition of $\UTxO$ and $\Coin$ abstract, and place constraints
on the choice of these types.

Regarding $\UTxO$, we require that $(\UTxO, \cup, \cap)$ is a group, where
$\cup$ is the addition operation, $\cap$ is the multiplication operation, and
$\emptyset$ is the additive identity. Given $u \in \UTxO$ we will write
$\overline{u}$ for the inverse element of $u$. Given $u, v \in \UTxO$, we write
$u \setminus v$ as a shorthand for $\overline{\overline{u} \cup v}$.

Additionally, we require that the elements in $\UTxO$ form a preorder w.r.t. a
relation $\subseteq$.

Note that the notation chosen suggests that $\UTxO$ is a set, however this does not
necessarily has to be the case.

As for $\Coin$ the only requirement in these rules is that the elements of this
set form a total order w.r.t a relation $\leq$.
...
\begin{figure*}[h]
  ...
  \begin{align*}
    ...
    & (\UTxO, \subseteq)~\text{is a total order} & \text{total order on }\UTxO\\
    & (\UTxO, \cup, \emptyset) & \text{is a group}
  \end{align*}
