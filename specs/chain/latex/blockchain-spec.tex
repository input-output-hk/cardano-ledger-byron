...
A new block $b$ with an index $ix \in BlockIx$, signed by a signing key
$sk_d$ in a slot $sl$ can extend a blockchain if the following conditions are
met:
%
\begin{enumerate}
...
\marko{We need a map (denoted as \signmapname in
  ...
  we push the block's index to the queue. We pop from the queue if the head of
  the queue is more than $K$ blocks away from $ix$.}
