...
\newcommand{\delegationname}{delegation} % the delegation relation
...
\newcommand{\postnewcerts}[2]{\fun{\postnewcertsname} ~ #1 ~ #2}
...
%
This is modeled as a relation
$\fun{\postnewcertsname} \subseteq (\powerset{\HCert}) \times \Slot$.
%
...
%
That is why new certificates can be added to the relation in the current slot
only if there are no conflicts with existing certificates that are stored in
the ledger layer.
%
...
%
In case a certificate cannot be added to the relation, the block to be added
to the blockchain is discarded.
...
indirectly via delegation), as given in Figure~\ref{fig:init-bc-state}.
...
\begin{figure}
  ...
  \begin{align*}
    ...
    \fun{\delegationname} & \in \VKeyGen \times \VKey \times \Slot
      & \text{delegation relation}\\
    \fun{\postnewcertsname} & \in (\powerset{\HCert}) \times \Slot
      & \text{new certificates relation}
  \end{align*}
  %
...
\begin{figure}
  ...
  \inference[Blockchain-extension]
  {
    {\begin{array}{c}
      ...
      \hash{b} = \hashofblock{p} \\
      \delegation{vk_s}{vk_d}{sl} \wedge \verify{vk_d}{(\bdata{b})}{(\bsig{b})} \\
      \size{\signmap{vk_s}} \leq K \cdot t \\
      \signmapname' = \incixmap{vk_s}{(\trimix{\signmapname}{ix})}{ix} \\
      \postnewcerts{(\bcerts{b})}{sl} \\
      p' = b 
    \end{array}}
  }
  {
