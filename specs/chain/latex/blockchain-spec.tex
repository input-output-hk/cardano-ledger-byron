...
the genesis block signed a block (neither directly or indirectly via
delegation), as given by \eqref{eq:init-signed-blocks}.

\begin{equation}
  \label{eq:init-signed-blocks}
  \inference[Initial-signing-map]
  {}
  {\signmapname = \emptyset}
\end{equation}

Map $\signmapname$'s type is given in Figure~\ref{fig:block-ext-types-funs}.
...
\marko{We need a map (denoted as $\signmapname$ in
  Figure~\ref{fig:blockchain-extension}) from genesis block verification keys
  to a queue of block indices. For a new block to be added to the blockchain,
  we push the block's index to the queue. For each queue, we pop from the
  queue if the head of the queue is more than $K$ blocks away from $ix$.}
...
\begin{figure}
  ...
  {
    {\begin{array}{c}
      vk_s = \delegator{vk_d}{sl} \wedge \size{\signmap{vk_s}} \leq K \cdot t \\
      \signmapname' = \incixmap{vk_s}{(\trimix{\signmapname}{ix})}{ix} \\
      \mathcal{C} = \bcerts{b} \\
      \postnewcerts{\mathcal{C}}
    \end{array}}
  }
  {\signmapname \trans{}{b} \signmapname'}
  \end{equation*}
