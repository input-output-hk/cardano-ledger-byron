...
\newcommand{\SubSo}{\type{SubS}_1}
\newcommand{\SubSt}{\type{SubS}_2}
...
defined rules governing the transitions of distinct parts (substates), one (most
...
rule to reflect. In particular, let us consider two substates, $\SubSo$ and
$\SubSt$, with $A, B \in \SubSo$ and $A', B' \in \SubSt$, with all other
notation as in the Example inference rule above. If we have one rule for
the transitions of $\SubSo$, triggered by $z$, and one transition rule for $\SubSt$,
triggered by signal $z'$, to describe every transition
of the form  $(A, A') \to (B, B')$,  we would make two rules:

\[
  \inference[ExampleS1]
    {P(A) & z = f ~ s & \tion[E1_{s1}]{S1}{A}{z}{B}}
    {\tion[E1]{S2}{(A,A')}{s}{(B,A')}}
\]
and
\[
  \inference[ExampleS2]
    {P(A') & z' = f' ~ s' & \tion[E1_{s2}]{S1'}{A'}{z'}{B'}}
    {\tion[E1]{S2'}{(A,A')}{s'}{(A,B')}}
\]

where $E1 = E1_{s1} \cup E1_{s2}$. These rules can be applied in sequence,
to give the full $(A,A') \to (B,B')$ transition.
...
same type as $s$ and $s'$ (which is not the case if, for example,
$s$ and $s'$ are certificates and we do not specify some way to build
a new certificate that includes both),
it does not seem like we can get away with
having just one rule (or it may be the most straighforward option to
describe the system, anyway).

When the behaviour of the substates is
independent of each other, giving the rules ExampleS1 and ExampleS2 is
sufficient to describe every transition.
\end{todo}
