...
import           Test.Cardano.Chain.Elaboration.Block        (AbstractToConcreteIdMaps(..),
                                                              abEnvToCfg
                                                              )
...
setupChainValidationState sampleTrace =
  let chainEnv@(_, abstractInitialUTxO,_,_,_) = STS._traceEnv sampleTrace
      ...
      (cvs, abstractToConcreteIdMaps) = either (panic . show) identity $
        foldM
...
ts_mempoolValidation = withTestsTS 100 . property $ do
  ...
  let (Spec.Slot slot,utxo0,_,_,blockCount) = STS._traceEnv sampleTrace
      abstractChainState@
        (_stateSlot, allowedDelegators, h, utxoState, diState, upiState)
  ...
  let abstractBlock = STS.mkBlock h nextSlot issuerKey aBlockVersion
        (maybeToList dcert) uProp vote [transaction]
      concreteBlock = elaborateBlock config cvs abstractToConcreteIdMaps
        abstractChainState abstractBlock
