...
module Test.Cardano.Crypto.CryptoSpec
       ( spec
       ) where
...
randomSpec =
  describe "Crypto"
    ...
    $ do
        ...
        specify "[0,1)"
          $          Crypto.deterministic seed (Crypto.randomNumber 1)
          `shouldBe` 0
            -- specify "[0,2)" $
