...
mkScryptParams sp = S.scryptParamsLen
  (fromIntegral $ spLogN sp)
  (fromIntegral $ spR sp)
  (fromIntegral $ spP sp)
  (fromIntegral $ spHashLen sp)
...
encryptPass
  :: (Bi pass, MonadRandom m) => S.ScryptParams -> pass -> m S.EncryptedPass
encryptPass params passphrase =
  genSalt <&> \salt -> encryptPassWithSalt params salt passphrase

encryptPassWithSalt
  :: Bi pass => S.ScryptParams -> S.Salt -> pass -> S.EncryptedPass
