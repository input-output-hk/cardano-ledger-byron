...
instance Bi BlockHeader where
   ...
   decode = do
       ...
       case t of
           ...
           _ -> cborError $ "decode@BlockHeader: unknown tag " <> sformat build t
...
mkBoundaryHeader pm prevHeader epoch body = GenericBlockHeader
  ...
  where
    ...
    consensus  = BoundaryConsensusData
        { _gcdEpoch      = epoch
...
verifyMainBlockHeader pm gbh = do
  ...
  where
    ...
    verifyBlockSignature (BlockPSignatureLight proxySig) = proxyVerify
        ...
        signature
    verifyBlockSignature (BlockPSignatureHeavy proxySig) = proxyVerify
        pm
        SignMainBlockHeavy
        proxySig
        (const True)
...
verifyMainConsensusData mcd = when (selfSignedProxy $ _mcdSignature mcd)
  ...
  where
    ...
    selfSignedProxy (BlockPSignatureLight sig) =
        isSelfSignedPsk $ psigPsk sig
    selfSignedProxy (BlockPSignatureHeavy sig) =
        isSelfSignedPsk $ psigPsk sig
