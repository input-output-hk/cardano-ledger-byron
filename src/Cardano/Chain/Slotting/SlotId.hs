...
unflattenSlotId es n = SlotId
    { siEpoch = fromIntegral epoch
    -- We can use @UnsafeLocalSlotIndex@ as we ensure @slot < es@
    , siSlot = UnsafeLocalSlotIndex $ fromIntegral slot
    }
  where (epoch, slot) = n `divMod` fromIntegral es
...
crucialSlot k epochIdx = SlotId {siEpoch = epochIdx - 1, siSlot = slot}
  where
    -- We can use @UnsafeLocalSlotIndex@ here as we ensure @slot < epochSlots@
    slot =
