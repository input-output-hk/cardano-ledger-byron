  ...
  where
    checkOutput i txOut = do
        unless (txOutValue txOut > Coin 0) $ throwError $ sformat
            ...
            i
            (txOutValue txOut)
        unless (isRight . checkCoin $ txOutValue txOut)
            $ throwError
            $ sformat ("output #" % int % " has invalid coin") i
...
instance Bi TxIn where
    encode (TxInUtxo txInHash txInIndex) = encodeListLen 2
        <> encode (0 :: Word8)
        <> encodeKnownCborDataItem (txInHash, txInIndex)
    encode (TxInUnknown tag bs) = encodeListLen 2
        <> encode tag
...
txInFromText t = case T.splitOn "_" t of
    ["TxInUtxo", h, idx] ->
        TxInUtxo <$> decodeAbstractHash h <*> readEither idx
    ["TxInUnknown", tag, bs] ->
...
instance FromJSON TxOut where
    parseJSON = withObject "TxOut" $ \o -> TxOut
...
instance Bi TxOut where
    encode txOut = encodeListLen 2
        <> encode (txOutAddress txOut)
        <> encode (txOutValue txOut)
